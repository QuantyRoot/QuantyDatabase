...
fn main() {
    ...
    let dir = scratch_dir();
    println!("quantydb:  {}", quantydb.display());
    ...
    let prepared = [
        (Engine::Quanty, prepare(&dir, &quantydb, Engine::Quanty)),
...
fn prepare(dir: &Path, binary: &Path, engine: Engine) -> PathBuf {
    let (db, script, args) = match engine {
        Engine::Quanty => {
            ...
            std::fs::write(&script, quantydb_insert(Mode::Bulk)).expect("writing the script");
    ...
    for run in 0..REPEATS {
        ...
        let (db, script, args) = match engine {
            Engine::Quanty => {
                ...
                std::fs::write(&script, (workload.quantydb)(mode)).expect("writing the script");
                (db.clone(), script, vec!["shell".to_string(), path(&db)])
            }
            Engine::Sqlite => {
                let db = match workload.phase {
...
fn quantydb_insert(mode: Mode) -> String {
...
fn quantydb_lookup(_mode: Mode) -> String {
    let mut s = String::new();
...
fn quantydb_scan(_mode: Mode) -> String {
    let mut s = String::new();
    for _ in 0..SCANS {
...
fn quantydb_indexed(_mode: Mode) -> String {
    let mut s = String::new();
    for key in lookup_keys() {
        let _ = writeln!(
            s,
...
fn print_table(rows: &[(Phase, &str, String, Duration, Duration)]) {
    println!(
        "{:<9} {:<9} {:<38} {:>10} {:>10} {:>9}",
        "phase", "workload", "what", "quantydb", "sqlite", "ratio"
    );
    println!("{}", "-".repeat(89));
    ...
    println!(
        "\nratio is quantydb divided by sqlite: below 1.00 is faster than sqlite, \
    ...
    for (phase, name, _, ours, theirs) in rows {
        println!(
            "RESULT {phase} {name} quantydb_ms={:.1} sqlite_ms={:.1} ratio={:.3}",
...
/// The quantydb binary that was built alongside this one.
...
fn scratch_dir() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("quantydb-bench-{}-{nanos}", std::process::id()));
