...
use quantydb_core::{Storage, Value};
use quantydb_exec::{ExecError, Output, Session};
use quantydb_proto::{batch_rows, ClientMessage, ErrorCode, ServerMessage};
use quantydb_ql::ast::Statement;
use quantydb_ql::ParseError;
...
pub(crate) fn parse(request: &ClientMessage) -> Result<Option<Parsed>, Vec<ServerMessage>> {
    let parsed = match request {
        ...
        ClientMessage::Query(source) => quantydb_ql::parse(source),
        ClientMessage::QuerySql(source) => quantydb_ql::parse_sql(source),
