...
use quantydb_auth::{sha256, to_hex, Tokens};
use quantydb_proto::{ErrorCode, ServerMessage};
use quantydb_service::Deadlines;
...
impl TokenFile {
    fn with(name: &str, tokens: &[&str]) -> TokenFile {
        let path = std::env::temp_dir().join(format!("quantydb-svc-auth-{name}"));
