...
use common::TestDir;
use quantydb_core::{Db, Value};
use quantydb_exec::Session;
use quantydb_import::{execute, plan, Options};
use quantydb_sqlite::{FileSource, Reader, SliceSource};
...
fn import(
    fixture: &str,
    dir: &TestDir,
) -> (Session<quantydb_core::FileStorage>, quantydb_import::Report) {
...
/// returns them.
fn column(
    session: &mut Session<quantydb_core::FileStorage>,
    table: &str,
    column: &str,
) -> Vec<String> {
    let output = session
...
fn a_database_that_needs_its_log_imports_from_both_files() {
    let dir = TestDir::new();
    let reader = quantydb_sqlite::open_file(data_path("wal_mode.sqlite")).unwrap();
    let plan = plan(&reader, &Options::default()).unwrap();
...
fn imported_rows(
    ...
    {
        quantydb_exec::Output::Rows { rows, .. } => rows,
...
fn source_rows(
    ...
    plan: &quantydb_import::ImportPlan,
    table: &quantydb_import::TablePlan,
) -> Vec<Vec<Value>> {
    let schema = reader.schema().unwrap();
    let object = schema.object(&table.source_name).unwrap();
    let def = object.table_def().unwrap();
    let layout = quantydb_sqlite::RowLayout::new(&def);
    let _ = plan;
    ...
    for row in reader.rows(table.root_page).unwrap() {
        ...
        for column in &table.columns {
            let value = match column.source {
                quantydb_import::ValueSource::Rowid => Value::Int(row.rowid.unwrap()),
                quantydb_import::ValueSource::Declared(index) => match layout.cell(&row, index) {
                    quantydb_sqlite::MappedCell::Rowid(id) => Value::Int(id),
                    quantydb_sqlite::MappedCell::Missing => {
                        column.default.clone().unwrap_or(Value::Null)
                    }
                    quantydb_sqlite::MappedCell::Virtual => panic!("a virtual column was planned"),
                    quantydb_sqlite::MappedCell::Value(value) => convert(value, column.ty),
                },
            };
            values.push(value);
        }
...
fn convert(value: &quantydb_sqlite::SqliteValue, ty: quantydb_ql::ast::TypeName) -> Value {
    use quantydb_ql::ast::TypeName;
    use quantydb_sqlite::SqliteValue as S;
    match (value, ty) {
        (S::Null, _) => Value::Null,
