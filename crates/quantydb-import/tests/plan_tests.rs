...
use quantydb_import::{plan, ImportPlan, Note, Options, Problem, ValueSource};
use quantydb_ql::ast::TypeName;
use quantydb_sqlite::{Reader, SliceSource};
...
fn table<'a>(plan: &'a ImportPlan, name: &str) -> &'a quantydb_import::TablePlan {
    plan.tables
        .iter()
...
fn column<'a>(
    table: &'a quantydb_import::TablePlan,
    name: &str,
) -> &'a quantydb_import::ColumnPlan {
    table
        .columns
        .iter()
...
fn a_table_without_a_usable_key_gets_its_rowid() {
    ...
    let key: Vec<&quantydb_import::ColumnPlan> = nopk.key_columns().collect();
    assert_eq!(key.len(), 1);
    ...
    let key: Vec<&quantydb_import::ColumnPlan> = nullable_pk.key_columns().collect();
    assert_eq!(key.len(), 1);
    assert_eq!(key[0].source, ValueSource::Rowid);
...
fn a_default_fills_the_rows_that_predate_its_column() {
    ...
    assert_eq!(
        b.default,
        Some(quantydb_core::Value::Text("fallback".into()))
    );
