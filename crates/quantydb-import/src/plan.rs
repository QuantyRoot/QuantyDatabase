...
use quantydb_core::Value;
use quantydb_ql::ast::TypeName;
use quantydb_sqlite::{ColumnSurvey, Reader, SchemaObject, Source, StorageClass, TableSurvey};
...
pub fn plan<S: Source>(
    reader: &Reader<S>,
    options: &Options,
) -> quantydb_sqlite::Result<ImportPlan> {
    ...
    for object in schema.objects() {
        ...
        match object.kind {
            quantydb_sqlite::ObjectKind::Table => plan_table(
                reader,
                object,
                options,
                &indexes,
            ...
            quantydb_sqlite::ObjectKind::Index => {}
...
fn note_constraints(def: &quantydb_sqlite::TableDef, table: &str, plan: &mut ImportPlan) {
    use quantydb_sqlite::Constraint;
...
fn key_limit() -> usize {
    quantydb_core::max_key_len(quantydb_core::PagerOptions::default().page_size)
}

fn choose_key(def: &quantydb_sqlite::TableDef, survey: &TableSurvey) -> KeyChoice {
