//! The claims `quantydb about` makes, checked against the repository.
//!
...
fn the_lock_file_holds_nothing_but_this_workspace() {
    ...
    let foreign: Vec<&String> = packages
        .iter()
        .filter(|n| !n.starts_with("quantydb"))
        .collect();
    assert_eq!(
        foreign.len(),
        quantydb_cli_claims::FOREIGN_DEPENDENCIES,
...
fn about_never_claims_more_crates_than_exist() {
    let crates = fs::read_dir(root().join("crates"))
        ...
        .count();
    assert!(
        crates >= quantydb_cli_claims::CRATES,
        "about claims {} crates, {crates} exist",
        quantydb_cli_claims::CRATES
    );
    assert!(about().contains(&format!("crates         {}", quantydb_cli_claims::CRATES)));
}
...
fn about_never_claims_more_tests_than_exist() {
    ...
    assert!(
        tests >= quantydb_cli_claims::TESTS,
        "about claims {} test functions, {tests} exist",
        quantydb_cli_claims::TESTS
    );
}
