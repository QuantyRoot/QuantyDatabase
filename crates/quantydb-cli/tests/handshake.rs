...
use quantydb_proto::{ClientHello, Refusal, ServerHello, SERVER_HELLO_LEN, VERSION};

use common::TestDir;

fn quantydb(args: &[&str]) -> std::process::Output {
...
fn server() -> (TestDir, Served) {
    ...
    assert!(quantydb(&["run", &path, "table t { id: int @key }"])
        .status
        .success());
...
fn a_refused_connection_does_not_spoil_the_next_one() {
    ...
    let out = quantydb(&["connect", &served.addr, "show tables"]);
    assert!(
        out.status.success(),
        "{}{}",
