...
/// `setup` wrote.
const UNIT: &str = "/etc/systemd/system/quantydb.service";
...
pub fn setup(
    ...
    let database = match database {
        ...
        None => PathBuf::from(ask("database file", "quantydb.qdb", yes)?),
    };
    let tokens = match tokens {
        Some(given) => PathBuf::from(given),
        None => PathBuf::from(ask("token file", "quantydb.tokens", yes)?),
    };
    ...
    let (token, line) = quantydb_auth::mint("first")
    ...
    emit("  Losing it means minting another with `quantydb token <label>`.")?;
    emit("")?;
    ...
    if wants_service {
        ...
        emit("  systemctl enable --now quantydb")?;
    } else {
        emit("to start it:")?;
        emit(&format!(
            "  quantydb serve {} --listen {listen} --tokens {}",
            database.display(),
            tokens.display()
        ))?;
    }
    emit("")?;
    emit("to connect:")?;
    emit(&format!("  quantydb connect {listen} --token <token>"))?;
    emit("")?;
    emit("`quantydb uninstall` removes the service and the binary, and never")?;
...
pub fn uninstall(yes: bool) -> Result<(), Failure> {
    ...
    if has_unit {
        ...
        for args in [["disable", "--now"], ["reset-failed", ""]] {
            ...
            let _ = command.arg("quantydb").status();
        }
...
fn write_unit(database: &Path, tokens: &Path, listen: &str) -> Result<(), Failure> {
    ...
    if unit.exists() {
        return Err(failed(format!(
            "{UNIT} already exists. Remove it with `quantydb uninstall`, or \
