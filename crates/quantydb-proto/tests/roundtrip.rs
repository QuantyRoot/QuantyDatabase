...
use quantydb_proto::codec::Writer;
use quantydb_proto::error::ProtoError;
use quantydb_proto::frame::{FrameHeader, HEADER_LEN};
use quantydb_proto::message::{T_QUERY, T_ROW_BATCH};
use quantydb_proto::value::{encoded_row_len, encoded_value_len};
use quantydb_proto::write_value;
use quantydb_proto::{
...
fn encoded_len_matches_encoder() {
    ...
    let mut w = Writer::new();
    quantydb_proto::value::write_row(&mut w, &row);
    assert_eq!(w.finish().unwrap().len(), encoded_row_len(&row));
}

#[test]
fn a_count_above_the_protocol_cap_is_refused() {
    use quantydb_proto::{MAX_ROWS_PER_BATCH, MAX_VALUES_PER_ROW};
...
fn terminal_messages_are_the_ones_that_end_a_request() {
    ...
    assert!(ServerMessage::error(quantydb_proto::ErrorCode::Parse, "x").is_terminal());
