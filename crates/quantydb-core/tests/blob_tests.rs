...
use common::TestDir;
use quantydb_core::{hash_chunk, BlobRef, Db};
...
fn tx_refs(db: &Db<quantydb_core::FileStorage>, hash: &quantydb_core::ChunkHash) -> u64 {
...
fn round_trip(db: &Db<quantydb_core::MemStorage>, data: &[u8]) -> BlobRef {
...
fn a_blob_crossing_a_commit_boundary_round_trips() {
    ...
    let db = Db::in_memory().unwrap();
    let chunks = quantydb_core::CHUNKS_PER_COMMIT + 1;
    let len = chunks * quantydb_core::CHUNK_SIZE + 1234;
...
fn short_reads_do_not_move_where_chunks_end() {
    ...
    let db = Db::in_memory().unwrap();
    let data = pattern(2 * quantydb_core::CHUNK_SIZE + 500);
...
fn a_written_blob_is_all_unclaimed_until_something_names_it() {
    ...
    let db = Db::in_memory().unwrap();
    let data = pattern(3 * quantydb_core::CHUNK_SIZE);
    let blob = db.write_blob(&data[..]).unwrap();
...
fn releasing_the_last_reference_leaves_the_store_sound() {
    let db = Db::in_memory().unwrap();
    let data = pattern(2 * quantydb_core::CHUNK_SIZE);
    let blob = db.write_blob(&data[..]).unwrap();
...
fn the_checker_finds_a_chunk_that_is_only_half_there() {
    ...
    use quantydb_core::{encode_key, Value};
