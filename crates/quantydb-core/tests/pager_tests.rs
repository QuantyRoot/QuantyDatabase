...
use quantydb_core::{Error, FileStorage, MemStorage, PageType, Pager, PagerOptions};
...
fn fill_body(buf: &mut [u8], seed: u8) {
    for (i, b) in buf
        .iter_mut()
        .enumerate()
        .skip(quantydb_core::PAGE_HEADER_LEN)
    {
...
fn check_body(buf: &[u8], seed: u8) {
    for (i, b) in buf.iter().enumerate().skip(quantydb_core::PAGE_HEADER_LEN) {
...
fn bulk_load_one_million_pages_in_memory() {
    ...
    for _ in 0..20 {
        ...
        for _ in 0..chunk {
            ...
            body[quantydb_core::PAGE_HEADER_LEN] = (id % 251) as u8;
        }
        batch.commit().unwrap();
    }
    let meta = pager.committed_meta();
    ...
    for id in [2u64, 123_456, 999_999] {
        ...
        assert_eq!(page[quantydb_core::PAGE_HEADER_LEN], (id % 251) as u8);
    }
}
