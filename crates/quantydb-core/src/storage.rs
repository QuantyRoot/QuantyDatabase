...
impl FileStorage {
    ...
    fn claim(file: File) -> Result<Self> {
        match quantydb_sys::lock::try_write_lock(&file) {
            Ok(()) => Ok(FileStorage { file, locked: true }),
            Err(quantydb_sys::lock::LockError::Held) => Err(Error::AlreadyOpen),
            ...
            // help.
            Err(quantydb_sys::lock::LockError::Unsupported(_)) => Ok(FileStorage {
                file,
                locked: false,
            }),
        }
