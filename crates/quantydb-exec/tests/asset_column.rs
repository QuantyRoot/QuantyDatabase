...
use quantydb_core::{BlobRef, Db, MemStorage};
...
fn a_row_that_names_a_blob_claims_its_chunks() {
    let mut s = session();
    let blob = stored(&s, &pattern(3 * quantydb_core::CHUNK_SIZE));
...
fn deleting_the_row_gives_the_chunks_back() {
    let mut s = session();
    let blob = stored(&s, &pattern(2 * quantydb_core::CHUNK_SIZE));
    s.execute(&format!("put files {{ id: 1, body: x\"{}\" }}", hex(&blob)))
        .expect("put");
    assert_eq!(sound(&s), 2);
...
fn two_rows_naming_one_blob_both_have_to_go() {
    let mut s = session();
    let blob = stored(&s, &pattern(quantydb_core::CHUNK_SIZE));
...
fn overwriting_swaps_one_blob_for_another() {
    let mut s = session();
    let first = stored(&s, &pattern(quantydb_core::CHUNK_SIZE));
    let second = stored(&s, &pattern(2 * quantydb_core::CHUNK_SIZE + 7));
...
fn dropping_the_table_releases_what_its_rows_held() {
    ...
    let mut s = session();
    for i in 1..=3 {
        let blob = stored(&s, &pattern(quantydb_core::CHUNK_SIZE + i));
        s.execute(&format!(
...
fn a_blob_nobody_stored_is_collected() {
    ...
    let mut s = session();
    let orphan = stored(&s, &pattern(2 * quantydb_core::CHUNK_SIZE));
...
fn a_blob_a_row_names_survives_the_sweep() {
    let mut s = session();
    let kept = stored(&s, &pattern(quantydb_core::CHUNK_SIZE));
    let orphan = stored(&s, &pattern(3 * quantydb_core::CHUNK_SIZE + 11));
...
fn sweeping_twice_drops_nothing_the_second_time() {
    ...
    s.execute(&format!("put files {{ id: 1, body: x\"{}\" }}", hex(&blob)))
        .expect("put");
    stored(&s, &pattern(quantydb_core::CHUNK_SIZE + 3));
