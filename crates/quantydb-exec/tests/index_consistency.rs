...
fn rows(output: Output) -> Vec<Vec<quantydb_core::Value>> {
    match output {
...
fn run_sequence(seed: u64) {
    ...
    for tag in tags {
        ...
        let filtered: Vec<_> = all
            .iter()
            .filter(|row| row[1] == quantydb_core::Value::Text(tag.to_string()))
            .cloned()
            .collect();
        assert_eq!(
            indexed, filtered,
    ...
    for v in ["null", "0", "1", "2", "3", "4"] {
        ...
        let want: Vec<_> = all
            .iter()
            .filter(|row| match v {
                "null" => row[2] == quantydb_core::Value::Null,
                _ => row[2] == quantydb_core::Value::Int(v.parse().unwrap()),
            })
            .cloned()
            .collect();
