...
use quantydb_core::{decode_key, encode_key, Db, GcReport, Storage, SuspendedTx, Value, WriteTx};
use quantydb_ql::ast::{AsOf, ColumnRef, Direction, Expr, Get, Statement, TypeName};
...
impl<S: Storage> Session<S> {
    ...
    pub fn execute(&mut self, source: &str) -> Result<Output, ExecError> {
        let stmt = quantydb_ql::parse(source)?;
    ...
    pub fn execute_sql(&mut self, source: &str) -> Result<Output, ExecError> {
        let stmt = quantydb_ql::parse_sql(source)?;
        self.run_parsed(&stmt)
    }
...
trait View {
    fn view_get(&self, key: &[u8]) -> quantydb_core::Result<Option<Vec<u8>>>;
    fn view_scan(&self, start: Option<&[u8]>, end: Option<&[u8]>) -> Result<RawRows, ExecError>;
    fn view_catalog_get(&self, key: &[u8]) -> quantydb_core::Result<Option<Vec<u8>>>;
}
...
fn join_step<V: View>(
    ...
    for left in left_rows {
        ...
        if join.kind == quantydb_ql::ast::JoinKind::Left && !matched {
            let mut padded = left;
...
impl<S: Storage> Run<'_, S> {
    ...
    fn move_asset_refs(&mut self, value: &Value, delta: i64) -> Result<(), ExecError> {
        ...
        let blob = quantydb_core::BlobRef::decode(bytes)?;
    ...
    fn create_table(&mut self, def: &quantydb_ql::ast::TableDef) -> Result<Output, ExecError> {
        if self
            .tx
            ...
            for (_, values) in &rows {
                for &pos in &positions {
                    if let Value::Bytes(d) = &values[pos] {
                        for hash in quantydb_core::BlobRef::decode(d)?.chunks {
    ...
    fn put(&mut self, table_name: &str, rows: &[Vec<(String, Expr)>]) -> Result<Output, ExecError> {
        ...
        for row in rows {
            ...
            if !self.tx.put_unique(&key, &encode_key(&values))? {
                let pk = row_pk(&table, &values);
                let rendered: Vec<String> = pk.iter().map(quantydb_ql::pretty::literal).collect();
    ...
    fn set(
        ...
        table_name: &str,
        filter: Option<&Expr>,
        assigns: &[quantydb_ql::ast::Assign],
    ) -> Result<Output, ExecError> {
        let table = self.load_table(table_name)?;
            ...
            Statement::Get(get) if !get.joins.is_empty() => {
                ...
                if let Some(filter) = &get.filter {
                    node = ExplainNode::over(
                        format!("Filter {}", quantydb_ql::pretty::expr(filter)),
                        node,
                    );
                }
                if let Some((col, dir)) = &get.order {
