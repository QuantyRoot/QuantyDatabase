...
use quantydb_ql::ast::{BinaryOp, ColumnRef, Expr, TypeName, UnaryOp};
...
pub fn coerce(v: Value, ty: TypeName, nullable: bool) -> Result<Value, ExecError> {
    match (&v, ty) {
        ...
        (Value::Bytes(d), TypeName::Asset) => match quantydb_core::BlobRef::decode(d) {
            Ok(_) => Ok(v),
...
mod tests {
    ...
    fn v(src: &str) -> Result<Value, ExecError> {
        let stmt = quantydb_ql::parse(&format!("get t where {src}")).unwrap();
        let quantydb_ql::ast::Statement::Get(get) = stmt else {
            panic!()
        };
