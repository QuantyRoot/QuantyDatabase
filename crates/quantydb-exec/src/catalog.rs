...
use quantydb_ql::ast::{self, TypeName};
        ...
        for _ in 0..ncols {
            ...
            let default = match r.u8().ok_or_else(bad)? {
                ...
                1 => {
                    ...
                    let mut values = quantydb_core::decode_key(enc)
...
mod tests {
    ...
    fn table_roundtrips_through_the_codec() {
        let def = quantydb_ql::parse(
            "table users { id: int @key, name: text @index, score: float = 1.5, blob: bytes @null }",
        )
        .unwrap();
        let quantydb_ql::ast::Statement::TableDef(def) = def else {
            panic!()
        };
    ...
    fn validation_rejects_broken_definitions() {
        for src in [
            ...
            let quantydb_ql::ast::Statement::TableDef(def) = quantydb_ql::parse(src).unwrap()
            else {
                panic!()
            };
