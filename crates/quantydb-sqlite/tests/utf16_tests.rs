...
use quantydb_sqlite::{Reader, SliceSource, SqliteError, SqliteValue, TextEncoding};
...
fn a_text_value_of_odd_length_is_refused() {
    ...
    let err = quantydb_sqlite::decode_record(&payload, TextEncoding::Utf16Le)
    ...
    let value = quantydb_sqlite::decode_record(&payload, TextEncoding::Utf8).unwrap();
...
fn the_reader_supplies_the_encoding_itself() {
    ...
    while page.kind.is_interior() {
        page = match reader.cell(&page, 0).unwrap() {
            quantydb_sqlite::Cell::TableInterior { child, .. } => reader.btree_page(child).unwrap(),
            other => panic!("unexpected cell {other:?}"),
        };
    }
    let payload = match reader.cell(&page, 0).unwrap() {
        quantydb_sqlite::Cell::TableLeaf { payload, .. } => payload,
        other => panic!("unexpected cell {other:?}"),
    };

    assert_eq!(
        reader.decode_record(&payload).unwrap(),
        quantydb_sqlite::decode_record(&payload, TextEncoding::Utf16Be).unwrap()
    );
    // decoded as utf-16 the wrong way round, the same bytes are either an
    // error or a different string, never the same one
    let wrong = quantydb_sqlite::decode_record(&payload, TextEncoding::Utf16Le);
