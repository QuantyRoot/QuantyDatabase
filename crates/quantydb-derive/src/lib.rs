...
/// Derive [`quantydb::Row`] for a struct of named fields.
#[proc_macro_derive(Row, attributes(quantydb))]
...
/// One `#[...]` if the cursor is on one. Returns the inner `quantydb(...)`
...
fn attribute(tokens: &[TokenTree], i: &mut usize) -> Result<Option<Option<String>>, String> {
    ...
    *i += 2;
    ...
    match inner.first() {
        Some(TokenTree::Ident(id)) if id.to_string() == "quantydb" => {}
    ...
    let args = match inner.get(1) {
        ...
        _ => {
            return Err(
                "`#[quantydb]` needs a value, as in `#[quantydb(table = \"x\")]`".to_string(),
            )
        }
    };
...
fn named_string(stream: TokenStream, allowed: &[&str]) -> Result<Option<String>, String> {
    ...
    let key = match parts.first() {
        Some(TokenTree::Ident(id)) => id.to_string(),
        _ => return Err("`#[quantydb(...)]` starts with a name".to_string()),
    };
    if !allowed.contains(&key.as_str()) {
        return Err(format!(
            "`#[quantydb({key})]` is not understood; expected one of {}",
            allowed.join(", ")
        ));
    }
    match parts.get(1) {
        Some(TokenTree::Punct(p)) if p.as_char() == '=' => {}
        _ => return Err(format!("`#[quantydb({key})]` needs `= \"...\"`")),
    }
    match parts.get(2) {
        Some(TokenTree::Literal(l)) => unquote(&l.to_string())
            .map(Some)
            .ok_or_else(|| format!("`#[quantydb({key})]` needs a plain string")),
        _ => Err(format!("`#[quantydb({key})]` needs a string")),
    }
}
...
fn emit(model: &Model) -> TokenStream {
    ...
    let from: Vec<String> = model
        ...
        .map(|f| {
            format!(
                "            {}: ::quantydb::__private::field(\"{}\", \"{}\", it.next())?,",
                f.name, name, f.name
            )
        })
        .collect();
    ...
    let to: Vec<String> = model
        ...
        .map(|f| {
            format!(
                "            (\"{}\", ::quantydb::IntoValue::into_value(self.{}.clone())),",
                f.column, f.name
            )
        })
        .collect();
    ...
    let code = format!(
        "impl ::quantydb::Row for {name} {{
    const TABLE: &'static str = \"{table}\";
    const COLUMNS: &'static [&'static str] = &[{columns}];

    fn from_values(values: ::std::vec::Vec<::quantydb::Value>)
        -> ::quantydb::Result<Self>
    {{
        let mut it = values.into_iter();
        ::quantydb::Result::Ok({name} {{
{from}
        }})
    }}

    fn to_values(&self) -> ::std::vec::Vec<(&'static str, ::quantydb::Value)> {{
        ::std::vec![
{to}
        ]
    }}
