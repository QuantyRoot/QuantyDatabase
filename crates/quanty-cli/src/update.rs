...
/// `quanty` -> `quanty.new`, and `quanty.exe` -> `quanty.new.exe`.
///
...
fn version_of(program: &Path) -> Result<String, Failure> {
    ...
    if !out.status.success() {
        return Err(failed(format!(
            "{} ran and failed, so it is not a working quanty",
            program.display()
        )));
    }
    let text = String::from_utf8_lossy(&out.stdout);
    let first = text.lines().next().unwrap_or("").trim();
    let Some(version) = first.strip_prefix("quanty ") else {
        return Err(failed(format!(
            "{} is not quanty: it answered {first:?}",
            program.display()
        )));
    };
