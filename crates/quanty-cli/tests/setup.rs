...
fn own_copy(dir: &TestDir) -> PathBuf {
    let binary = dir.path().join(format!("quanty{EXE_SUFFIX}"));
    fs::copy(env!("CARGO_BIN_EXE_quanty"), &binary).expect("copy the tool");
    binary
}
...
fn setup_writes_a_database_a_token_and_says_how_to_start() {
    ...
    assert!(
        said.contains("quanty serve"),
        "did not say how to start it: {said}"
    );
    assert!(
        said.contains("quanty connect"),
