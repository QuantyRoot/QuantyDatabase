...
fn staged(dir: &TestDir) -> (PathBuf, PathBuf) {
    let installed = dir.path().join(format!("quanty{EXE_SUFFIX}"));
    let candidate = dir.path().join(format!("candidate{EXE_SUFFIX}"));
    fs::copy(env!("CARGO_BIN_EXE_quanty"), &installed).expect("copy the tool");
    fs::copy(env!("CARGO_BIN_EXE_quanty"), &candidate).expect("copy the candidate");
    (installed, candidate)
}
...
fn a_good_binary_is_installed_and_the_old_one_kept() {
    ...
    assert!(
        dir.path().join(format!("quanty.old{EXE_SUFFIX}")).exists(),
        "the old binary was not kept: {said}"
    );
    assert!(
        !dir.path().join(format!("quanty.new{EXE_SUFFIX}")).exists(),
