...
//!
//! ```no_run
//! # fn main() -> Result<(), quantydb::Error> {
//! let mut db = quantydb::Database::create("app.qdb")?;
...
//! }
//!
//! let mut db = quantydb::Database::open("app.qdb")?;
...
use quantydb_core::{FileStorage, MemStorage};
use quantydb_exec::{ExecError, Output, Session};
use quantydb_ql::ast::{Expr, Statement};

pub use quantydb_derive::Row;
...
///
/// ```compile_fail,E0499
/// let mut db = quantydb::Database::in_memory().unwrap();
/// db.transaction(|tx| {
...
impl Database {
    ...
    pub fn create(path: impl AsRef<Path>) -> Result<Self> {
        let db = quantydb_core::Db::create_file(path).map_err(Error::from_storage)?;
        Ok(Database {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        let db = quantydb_core::Db::open_file(path).map_err(Error::from_storage)?;
        Ok(Database {
            backend: Backend::File(Session::new(db)),
        })
    }
    ...
    pub fn in_memory() -> Result<Self> {
        let db = quantydb_core::Db::in_memory().map_err(Error::from_storage)?;
        Ok(Database {
...
fn into_core(v: Value) -> quantydb_core::Value {
    match v {
        Value::Null => quantydb_core::Value::Null,
        Value::Bool(b) => quantydb_core::Value::Bool(b),
        Value::Int(i) => quantydb_core::Value::Int(i),
        Value::Float(f) => quantydb_core::Value::Float(f),
        Value::Text(t) => quantydb_core::Value::Text(t),
        Value::Bytes(b) => quantydb_core::Value::Bytes(b),
    }
}
...
impl From<quantydb_core::Value> for Value {
    fn from(v: quantydb_core::Value) -> Self {
        match v {
            quantydb_core::Value::Null => Value::Null,
            quantydb_core::Value::Bool(b) => Value::Bool(b),
            quantydb_core::Value::Int(i) => Value::Int(i),
            quantydb_core::Value::Float(f) => Value::Float(f),
            quantydb_core::Value::Text(t) => Value::Text(t),
            quantydb_core::Value::Bytes(b) => Value::Bytes(b),
        }
    }
}
...
impl Error {
    ...
    fn from_storage(e: quantydb_core::Error) -> Self {
        Error {
